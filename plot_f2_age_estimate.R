...
sig.name <- "1"
regions.to.include <- c( "WestEurasia", "EastAsia", "SouthAsia", "America", "Oceania")
...
plot(all.med[inc], sig.med[inc], col=cols[reg[names(sig.med[inc])]], xlab=bquote("Median age of all mutations (generations)"), ylab=bquote("Median age of signature"~.(sig.name)~f[2]~"mutations (generations)"), pch=16, bty="n", ylim=c(100,300), xlim=c(150,350))
for(regi in regions.to.include){
  reg.mn <- mean(sig.med[reg[names(sig.med)]==regi]/all.med[reg[names(sig.med)]==regi], na.rm=T)
  abline(0, reg.mn, col=cols[regi])
  cat(paste0(regi, " ", reg.mn))
}
## abline(0,0.776, col="#E41A1C", lwd=2)
## abline(0,0.839, col="#4DAF4A", lwd=2)
 legend("bottomright", regions.to.include, col=c(cols[regions.to.include]), pch=c(rep(16, length(regions.to.include))), bty="n") 
